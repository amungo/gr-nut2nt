...
Nut2ntSettings Fx3Tuner::m_ntSettings;
...
// Reg2, 0x02
// D7-D2 Unused
// D1-D0 Mode<1:0> ["00"standby, "01"PLL "A" only, "10"PLL "A" only, "11"active]. Default "11" active
//--------------------------------------------------------------------
void Fx3Tuner::SetICMode(int _mode)
{
    uint8_t addr = 3;
    uint8_t bits_mode = _mode & 0x03;
    dev->send16bitSPI_ECP5( bits_mode, addr);
}

//-------------------------------------------------------------------------------------------
// PLL"A" Reg41, 0x29 / PLL"B" Reg45, 0x2
// D7-D2 Unused
// D1 PLL_#_Band. PLL#frequency band: "0"- L2/L3/L5 "1"- L1. Default: PLL "A" "1", PLL "B" "0"
// D0 PLL_#_EN. PLL#enable: "0"-disabled, "1"-enabled. Default "1"
//--------------------------------------------------------------------------------------------
void Fx3Tuner::SetPLL(int pll_idx, int band, int is_enable)
{
    ...
    pause();
    dev->send16bitSPI_ECP5( bit_full, addr );
    pause();
}
...
// Reg3, 0x03
// D7-D2: Unused
// D1: TCXO_sel TCXO frequency setting. "0"-10.0 MHz "1"-24.84 MHz
// D0: LO_Source LO source: "0"-PLL "A" for all channels
// "1" - PLL "A" for channels#1&2, PLL "B"for channels#3&4
// default "0"
//---------------------------------------------------
void Fx3Tuner::SetTCXO_LO(int tcxo_sel, int lo_src)
{
    ...
    dev->send16bitSPI_ECP5( reg, addr );
}
...
void Fx3Tuner::SetFreqDivs(int pll_idx, uint32_t N, uint32_t R)
{
    ...
    pause();
    dev->send16bitSPI_ECP5( regHi, addrHi );
    pause(5);
    dev->send16bitSPI_ECP5( regLo, addrLo );
    pause();
}
...
void Fx3Tuner::ConfigureClockLVDS(int clk_src, int clk_div, int ampl, int outDC) {
    ...
    dev->send16bitSPI_ECP5( ( div_bit << 0 ), 11 );
    dev->send16bitSPI_ECP5( (src_bit<<5)|(type_bit<<4)|(cc_bit<<2)|(ol_bit<<0), 12 );
    pause();
}
...
void Fx3Tuner::Set_AGC() {
    ...
    for ( int ch = 0; ch < 4; ch++ ) {
        dev->send16bitSPI_ECP5( reg15, 15 + ch*7 );
        dev->send16bitSPI_ECP5( reg16, 16 + ch*7 );
        dev->send16bitSPI_ECP5( reg18, 18 + ch*7 );
    }
}
...
void Fx3Tuner::Set_MGC(int rf_gain_idx, int ifa_coarse_gain_idx, int ifa_fine_gain_idx) {
    ...
    for ( int ch = 0; ch < 4; ch++ ) {
        dev->send16bitSPI_ECP5( reg15, 15 + ch*7 );
        dev->send16bitSPI_ECP5( reg17, 17 + ch*7 );
        dev->send16bitSPI_ECP5( reg18, 18 + ch*7 );
    }
}
